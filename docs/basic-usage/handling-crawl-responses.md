...
$response->transferStats();  // ?Eldernet\Crawler\TransferStatistics (transfer timing and metadata, null for faked responses)
...
Each response includes a `Eldernet\Crawler\TransferStatistics` object with typed accessors for timing data and other transfer details:
