...
For more control over request pacing, you can use a throttle. A throttle is a class that implements `Eldernet\Crawler\Throttlers\Throttle`. When a throttle is set, it takes precedence over the `delay` method.
...
```php
use Eldernet\Crawler\Crawler;
use Eldernet\Crawler\Throttlers\AdaptiveThrottle;
